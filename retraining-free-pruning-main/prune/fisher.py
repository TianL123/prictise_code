...
def collect_mask_grads(model, head_mask, neuron_mask, dataloader):
    ...
    head_grads = torch.stack(head_grads, dim=0) #64,12,12
    neuron_grads = torch.stack(neuron_grads, dim=0)  # 64,12,3072
    return head_grads, neuron_grads


@torch.no_grad()
def compute_fisher_info(grads):
    fisher_info = grads.pow(2).sum(dim=0) #64,12,12  -> 12,12
    return fisher_info
